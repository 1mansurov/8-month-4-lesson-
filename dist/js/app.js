...
const renderTransactions = () => {
    ...
    EXPENSES.forEach((transaction) => {
        ...
        li.innerHTML = `
            <div class="flex items-center justify-between p-6 align-middle  w-full rounded-lg h-auto ">
                <div class="flex flex-col">
                    <div class="text-xl text-white-700 font-bold">${transaction.transactionType || 'Тип не указан'}</div>
                    <div class="text-lg text-white-500 ml-2">${transaction.transactionName}</div>
                </div>
                <div class="text-right">
                    <div class="font-bold text-lg text-green-600">${transaction.transactionAmount} UZS</div>
                    <div class="text-sm text-white-700">${new Date(transaction.date).toLocaleTimeString()}</div>
                    <div class="flex space-x-4 mt-2">
                        <button class="text-green-500 font-semibold border border-green-500 px-2 py-1 rounded">Add</button>
                        <button class="text-red-500 font-semibold border border-red-500 px-2 py-1 rounded  " onclick="deleteTransaction(${transaction.date})">delete</button>
                    </div>
                </div>
            </div>
            `;
